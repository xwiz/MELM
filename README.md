...
- **Pi target**: Raspberry Pi 5 (8 GB). Measured baseline (laptop, Qwen2.5-0.5B GGUF): ~97 ms median TTFT, ~545 MB RSS during decode, ~61 tok/s. Core stdlib-only path: sub-millisecond latency, ~186 MB RSS. Raspberry Pi is an optional appliance validation target — all core acceptance tests run on any stdlib Python platform. Smoke-gate validation via `python -m melm pi-smoke` (~7 min).
