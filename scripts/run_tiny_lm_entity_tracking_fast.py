"""Evaluate saved tiny LM artifacts on local BabyLM 2026 fast entity tracking."""
...
from melm.training import score_tiny_lm_continuations
...
def main() -> None:
    ...
    parser.add_argument("--tokenizers", help="Optional comma-separated tokenizer names to include.")
    ...
    parser.add_argument("--out-json", default="reports/tiny_lm_entity_tracking_fast.json")
    parser.add_argument("--out-md", default="reports/tiny_lm_entity_tracking_fast.md")
    ...
    prompts = [case.prompt for case in cases for _option in case.options]
    completions = [option for case in cases for option in case.options]
    ...
    for manifest_path in sorted(root.glob("*/run_manifest.json")):
        ...
        if requested is not None and tokenizer.name not in requested:
            continue
        text_scores = score_tiny_lm_continuations(
            prompts,
            completions,
            tokenizer,
        ...
        report = evaluate_multiple_choice_scores(cases, score_by_text)
        runs.append(
            {
    ...
    print("Tiny LM BabyLM fast entity tracking")
