...
- agent-memory markets are already crowded, so MELM must prove a sharper advantage on state updates, contradiction handling, and evidence-gated abstention;
...
MELM should therefore be built as a phased validation ladder:

1. **MELM Guard.** Prove procedural working memory: rule extraction, rule caching, state tracking, policy/action validation, and evidence-backed allow/deny decisions.
2. **MELM Memory OS.** Prove hierarchical memory: working state, event memory, semantic memory, temporal updates, contradiction handling, and abstention against RAG and long-context baselines.
3. **MELM SLM Appliance.** Pair the runtime with a small model and measure whether explicit memory/rules reduce model size, token budget, and latency for constrained tasks.
4. **MELM Language R&D.** Continue morphology/tokenization and small-backbone work as compression and sample-efficiency research, not as the first commercial proof.
5. **MELM Integrated Model.** Train a 125M-370M integrated model only after the runtime wins independently; scale toward 800M only if the smaller gates justify it.
...
This is the MELM equivalent of a fast "left-brain" system, but the implementation should be described in engineering terms: procedural working memory plus production rules, state tracking, typed facts, and evidence-gated validation. Some questions should be answerable from this layer alone. Other questions should trigger retrieval from event memory, semantic memory, or a larger model.
...
morpheme-only. Familiar high-frequency forms should remain cheap whole lexical
units, ordinary subword variation should use a strong statistical tokenizer such
as Unigram, and explicit morphology should activate when it preserves useful
structure without imposing excessive sequence length. This is closer to how the
rest of MELM treats memory: what is frequently or recently useful should be
retrieved quickly, while less familiar material should trigger deeper
decomposition.
...
- the benefits must be measured against strong baselines.
...
MELM must not claim human-like child cognition from isolated language tests.
...
The MELM risk is that English is comparatively analytic. A 12,000-morpheme vocabulary might be enough for the intended conversational scope, but it is not a safe assumption. MELM must test 4k, 8k, 12k, 20k, BPE, Unigram, and byte/patch alternatives before locking a tokenizer.

### 3.2 Tokenizer-free and byte/patch competitors

MELM must compete against the strongest alternative to "better linguistic tokens": reducing or removing tokenization assumptions entirely.
...
These systems are direct competitors to the morpheme hypothesis. If byte/patch models win at similar compute and model size, MELM should pivot toward event memory and treat morphology as auxiliary supervision rather than a required tokenizer.
...
The MELM correction is important: EM-LLM should be treated first as an external-memory baseline and an event-segmentation prior. Integrating event tokens into a trainable backbone is a research contribution, not glue work.
...
The first validation target is 125M-370M. An 800M model should wait until the smaller ablations justify scaling.

### 3.5 Developmentally plausible pretraining

BabyLM is the right evaluation culture for MELM because it constrains data and rewards careful corpus construction. MELM should use BabyLM 10M/100M-style tracks where possible, plus controlled synthetic event-dialogue data only after data quality filters are in place.

Synthetic data should be grounded in event records. Free-generation synthetic dialogue is too likely to add noise. Teacher models should be provider-agnostic; current model IDs and pricing must be checked at generation time.
...
MELM's differentiator should not be "it has memory" or "it has rules." The differentiator should be the combination: a procedural working-memory layer that can decide when local rules are enough, when event/state memory is required, when semantic memory is required, and when the system must abstain.
...
However, `cargo test --no-default-features` timed out during planning, so that codebase should be treated as unverified until its build and test health are confirmed. The first MELM implementation should use Python equivalents for fast ML iteration. A Rust sidecar can be considered only after the Python event-memory prototype wins.
...
The first product-grade MELM layer is a rule-aware working-memory runtime. It should support:
...
This layer validates structure and state. It should not pretend to know facts that are not present. If local facts and rules are insufficient, it routes to event memory, semantic memory, a tool, or abstention.

### 4.2 Layer 2: Hierarchical event/state memory

The first event memory is external. It should store:
...
Access history should be treated as a retrieval prior, not as truth. Frequently
or recently recalled events may be faster to surface, but grounding and
contradiction checks must still decide whether the event is admissible evidence
for the current question.

The event store can begin with SQLite/Postgres plus FAISS or Qdrant. Property-graph tooling is optional and should be added only if graph traversal becomes a bottleneck.

### 4.3 Layer 3: Small model interface

The neural model should start as a parser, router, and response composer:
...
The first proof should use existing small open models or the current 23.3M local checkpoint scaffolding. Training a larger custom model is a later phase.
...
The original Tree-LSTM morpheme composition remains an interesting post-validation design. For the six-month demo, MELM should start simpler:
...
Synthetic data is allowed only when grounded in event records. Each generated sample should include:
...
Synthetic dialogue should fill measured gaps, not inflate the corpus.

### 5.3 Data quality metrics

The corpus pipeline should report:
...
Pass criterion: MELM Guard must beat prompt-only and schema-only baselines on false-allow rate, traceability, and latency on at least one realistic workflow.
...
Pass criterion: event/state memory beats standard RAG by at least 15% on controlled episodic recall and state-update tasks at matched context and retrieval budget.
...
Pass criterion: small model plus MELM runtime matches or beats a materially larger baseline on a constrained workflow while using less context, lower latency, or lower cost.
...
Human review may be used for conversation quality, but it must be paired with reproducible automatic tasks.
...
### Gate 1: 14-day procedural working-memory smoke
...
Pass criterion: the runtime blocks invalid actions and allows valid actions more reliably than prompt-only control, with every decision grounded in an explicit rule or fact.

### Gate 2: 30-day event/state memory proof
...
Pass criterion: event/state memory beats the best RAG baseline by at least 15% on state-update and temporal-reasoning tasks while maintaining a lower false-answer rate through abstention.

### Gate 3: 60-day small-model runtime proof
...
Pass criterion: small model plus MELM runtime matches or beats a larger-model baseline on a constrained workflow at lower cost or latency.

### Gate 4: 90-day external benchmark demo
...
Pass criterion: an external reviewer can run the benchmark and reproduce the advantage without hand-editing the fixtures.

### Gate 5: 6-month language/model integration decision
...
| Months 4-5 | Language/model integration | Tokenizer and morphology ablations, optional 125M-370M run only if runtime gates pass |
...
- 800M public model unless smaller gates strongly justify it.
